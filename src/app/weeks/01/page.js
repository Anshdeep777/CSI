  ...
  return (
    <>
          ...
          <div className="relative p-[1px] overflow-hidden rounded-2xl bg-zinc-800/50 shadow-[0_8px_32px_0_rgba(0,0,0,0.5)]">
            ...
            <div className="relative bg-black/90 backdrop-blur-xl rounded-2xl p-4 text-center z-10">
              <p className="text-zinc-200 text-xs sm:text-sm font-medium tracking-wide leading-relaxed">
                Go through the flow, scroll down to see conversations, and
                access the relevant docs! Submission details are also mentioned
                below in chats.
              </p>
            </div>
          </div>
      ...
      <div className="flex min-h-screen bg-black w-full overflow-hidden mt-10">
        ...
        {/* MOBILE BOTTOM NAV */}
        <nav className="md:hidden fixed bottom-0 left-0 w-full bg-gradient-to-t from-black via-black/95 to-zinc-950/90 backdrop-blur-xl border-t border-cyan-500/30 rounded-t-[32px] z-50 pb-safe shadow-[0_-12px_30px_rgba(6,182,212,0.15),0_-4px_12px_rgba(6,182,212,0.1),inset_0_1px_2px_rgba(6,182,212,0.2)]">
          <div className="flex items-center justify-around px-2 py-3.5 relative z-10">
            <MobileTabButton
              active={pathname === "/"}
              onClick={() => handleNavigation("/weeks/01")}
        ...
        <main className="flex-1 md:ml-64 relative w-full pb-24 md:pb-10">
                ...
                <div className="relative group max-w-md w-full mt-8 md:mt-12">
                  ...
                  <div className="relative p-6 bg-[#060709]/95 border border-white/10 rounded-2xl shadow-[0_25px_60px_rgba(0,0,0,0.8)] overflow-hidden">
                    ...
                    <p className="text-sm text-gray-400 leading-relaxed font-medium mb-6">
                      We don't do standard PR or moral checkboxes—we engineer
                      mathematical survival solutions for corporations in
                      absolute crisis. The executive board is fractured, and the
                      ₹800 Cr funding freeze threatens the company's survival.
                      As a Junior Analyst in the GreenEdge Strategy Pod, your
                      immediate task is to strip away the corporate bias and map
                      the raw reality of Sprint's operational collapse.
                    </p>
                    ...
                    <div className="border-t border-white/5 pt-4 mb-5">
                      ...
                      <p className="text-xs text-gray-500 font-medium">
                        Deploy Systems Thinking frameworks to isolate the
                        invisible structural failures destroying Sprint's unit
                        economics. All Week 1 intelligence dossiers are now
                        unlocked and attached below.
                      </p>
                    </div>
                    ...
                    >
                      ...
                      <span className="text-green-400 group-hover:translate-x-0.5 transition-transform">
                        <BiDownArrow />
                      </span>
                    </button>
                  </div>
          ...
          <div
            ref={nextSectionRef}
            className="w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pb-16 sm:pb-24 pt-10 scroll-mt-12"
          >
            <div className="flex flex-col lg:flex-row gap-8 items-center lg:items-start justify-center">
              ...
              <div className="relative flex-1 w-full max-w-2xl p-5 md:p-6 bg-[#060709]/95 border border-cyan-500/30 rounded-2xl shadow-[0_20px_50px_rgba(239,68,68,0.05)]">
                ...
                <div className="mb-4 text-xs flex items-center gap-1.5">
                  ...
                  <span className="text-gray-200 font-bold tracking-wide truncate">
                    You're Hired. Here's the Situation.
                  </span>
                </div>
                ...
                <p className="text-sm md:text-base text-gray-300 leading-relaxed font-sans pl-3.5 border-l-2 border-cyan-500/40 bg-gradient-to-r from-cyan-500/[0.01] to-transparent py-1 pr-1">
                  Welcome to the team. Sprint is bleeding money, and GreenBridge
                  Capital blocked our ₹800 Cr Series B because of our ESG
                  metrics. I need your consulting pod to figure out what we are
                  actually dealing with. Start by understanding our business
                  inside and out. Let's talk today.
                </p>
              </div>
            </div>
          </div>
...
const TabButton = ({ active, onClick, icon, label, color }) => {
  ...
  return (
    ...
    >
      <div
        className={`${active ? colorMap[color].split(" ")[0] : `text-gray-500 group-hover:${colorMap[color].split(" ")[0]}`} transition-colors duration-300`}
      >
        {icon}
      </div>
      ...
      {active && (
        <span
          className={`ml-auto w-1.5 h-1.5 rounded-full animate-pulse ${colorMap[color].split(" ")[1]}`}
        />
      )}
    </button>
  );
...
const MobileTabButton = ({ active, onClick, icon, label, color }) => {
  ...
  return (
    ...
    >
      <div
        className={`${active ? colorMap[color].split(" ")[0] : "text-gray-500"} transition-colors duration-300`}
      >
        {icon}
      </div>
      <span
        className={`text-[9px] uppercase tracking-wider ${active ? "text-white font-bold" : "text-gray-500"}`}
      >
        {label}
      </span>
    </button>
